...
var cnItems = {
    ...
    "You": "你",
        ...
        'Y': 'Y', 'Z': 'Z',
    "100x Memory if Memory is less than 1e20": "记忆不足1e20时，记忆倍率×100",
    "Goal: Complete it.": "目标：完成该条件。",
    "Memory Deficiency": "记忆短缺",
    "Shards boosts themselves": "碎片增益效果自叠加",
    "Teleport": "传送",
    "Unlock a button in settings.": "在设置界面解锁一个按钮。",
    "Unlock Trials": "解锁试炼",
    "Upgrade Tree": "升级树",
    "This will reset Shards and Shard Upgrades [You need atleast 1e9 Shards]": "这将重置碎片和碎片升级 [你需要至少1e9个碎片]",
    "Energy": "能量",
    "Unlock Energy": "解锁能量",
    "Nightmare Trials": "梦魇实验",
    "Prestige Points boost Shard Gain": "声望点数提升碎片增益",
    "Automatically buy Shard Upgrades": "自动购买碎片升级",
    "Resets Everything at this point. [You need atleast 800 Prestige Points]": "此时将重置所有内容。[你需要至少800点声望点数]",
    "Remember.": "记住.",
    "Memory": "记忆",
    "Max All ": "全部最大 ",
    "Max All": "全部最大",
    "Exit?": "退出?",
    "Ascension": "转生",
    "Ascension Milestones": "转生里程碑",
    "Restart": "重新开始",
    "wake up": "唤醒",
    "You cant turn back?": "你不能回头吗？",
    "You woke up...": "你醒了...",
    "Unlock something new": "解锁新东西",
    "Thanks for playing Dream Incremental!": "感谢您玩梦境增量！",
    "Resets Everything at this point.": "在这一点上重置所有东西。",
    "Rebirth Points boost Shards": "重生点数提升碎片",
    "Passively Generate 10% of your Prestige Points per second": "每秒被动生成10%的声望点数",
    "Ascend": "转生",
    "": "",
    "": "",
    "": "",
...
var cnPrefix = {
    ...
    "Auto Rest: ": "自动休息: ",
    "Energy: ": "能量: ",
    "Shards Upgrades Autobuyer: ": "碎片升级自动购买器：",
    "Prestige Upgrades Autobuyer: ": "声望升级自动购买器：",
    "": "",
    "": "",
    "": "",
    "": "",
...
var cnRegReplace = new Map([
    ...
    [/^Unlock Toughts \[(.+)\] \[Permanent\]$/, '解锁思绪 [$1] [永久]'],
    [/^Unlock Thoughts \[(.+)\] \[Permanent\]$/, '解锁思绪 [$1] [永久]'],
    [/^You have (.+) points$/, '你有 $1 点数'],
    [/^Next at (.+) points$/, '下一个在 $1 点数'],
    [/^Prestige  \[(.+) Prestige Points\]$/, '声望  [$1 声望点]'],
    [/^Prestige \[(.+) Prestige Points\]$/, '声望  [$1 声望点]'],
    [/^Rebirth \[(.+) Rebirth Points\]$/, '重生  [$1 重生点数]'],
    [/^Jan ([\d\.,]+)$/, '1 月 $1'],
    [/^Feb ([\d\.,]+)$/, '2 月 $1'],
    [/^Mar ([\d\.,]+)$/, '3 月 $1'],
    [/^Apr ([\d\.,]+)$/, '4 月 $1'],
    ...
    [/^([\d\.]+) Thoughts$/, '$1 思绪'],
    [/^([\d\.]+) Memories$/, '$1 记忆'],
    [/^([\d\.]+)x Prestige Points$/, '$1x 声望点数'],
    [/^([\d\.]+)x Prestige Points.$/, '$1x 声望点数.'],
    [/^([\d\.]+)x Shards per Ascension starting at 3 Ascensions.$/, '$1x 碎片每次转生初始从 3 次转生开始.'],
    [/^([\d\.]+)x Thought$/, '$1x 思绪'],
    [/^([\d\.]+)x Nightmare$/, '$1x 梦魇'],
    [/^([\d\.]+)x Super Energy, \^([\d\.]+) Shards$/, '$1x 超级能量, ^$2 碎片'],
    [/^([\d\.]+)x Energy$/, '$1x 能量'],
    [/^([\d\.]+)x Energy, autobuy Prestige Upgrades$/, '$1x 能量, 自动购买声望升级'],
    [/^([\d\.]+)x Void$/, '$1x 虚空'],
    [/^([\d\.]+)x Rebirth Points$/, '$1x 重生点数'],
    [/^([\d\.]+)x Rebirth Points and unlock Super Energy$/, '$1x 重生点数 和 解锁超级能量'],
    [/^([\d\.]+)x Dreams$/, '$1x 梦境'],
    ...
    [/^([\d\.,]+)x Void if Void is less than 1e10$/, '$1x 虚空 如果虚空少于1e10'],
    [/^([\d\.,]+)x Void (.+)$/, '$1x 虚空 $2'],
    [/^([\d\.,]+)x Dreams (.+)$/, '$1x 梦境 $2'],
    ...
    [/^([\d\.,]+)x Shards$/, '$1x 碎片'],
    [/^([\d\.,]+)x Shards, ([\d\.,]+)x Prestige Points$/, '$1x 碎片，$2x 声望点数'],
    [/^([\d\.,]+)x Shards, ([\d\.,]+)x Prestige Points and ([\d\.,]+)x Rebirth Points$/, '$1x 碎片，$2x 声望点数 和 $3x 重生点数'],
    [/^([\d\.,]+)x Lucid Dreams$/, '$1x 清醒梦境'],
    [/^([\d\.,]+)x Lucid Energy$/, '$1x 清醒能量'],
    [/^([\d\.,]+) Lucid Dreams$/, '$1 清醒梦境'],
    [/^([\d\.,]+) Lucid Energy$/, '$1 清醒能量'],
    ...
    [/^World ([\d\.,]+)$/, '世界 $1'],
    [/^\+\^([\d\.]+) \#([\d\.,]+) Effect$/, '+^$1 #$2 效果'],
    [/^\+\^([\d\.]+) to \#([\d\.,]+) Effect$/, '+^$1 到 #$2 效果'],
    [/^Hyper boost \#([\d\.,]+)$/, '超级提升 #$1'],
    [/^Hyper Boost \#([\d\.,]+)$/, '超级提升 #$1'],
    [/^Great boost \#([\d\.,]+)$/, '大幅提升 #$1'],
    [/^Synergial Boost \#([\d\.,]+)$/, '协同提升 #$1'],
    [/^U-Prestige \#([\d\.,]+)$/, 'U-声望 #$1'],
    [/^U-Shards \#([\d\.,]+)$/, 'U-碎片 #$1'],
    [/^Powerful \#([\d\.,]+)$/, '强大 #$1'],
    [/^Prestiged-Energy \#([\d\.,]+)$/, '声望能量 #$1'],
    [/^Efficient \#([\d\.,]+)$/, '非常高效 #$1'],
    [/^Automated \#([\d\.,]+)$/, '自动化 #$1'],
    [/^Prestigeful \#([\d\.,]+)$/, '声望 #$1'],
    [/^Energetic \#([\d\.,]+)$/, '精力充沛 #$1'],
    [/^Exponential \#([\d\.,]+)$/, '指数 #$1'],
    [/^Energy is useful \#([\d\.,]+)$/, '能量是有用的 #$1'],
    [/^Basic \#([\d\.,]+)$/, '基础 #$1'],
    [/^Prepare \#([\d\.,]+)$/, '准备 #$1'],
    [/^Warmup \#([\d\.,]+)$/, '热身 #$1'],
    [/^Tenfolded \#([\d\.,]+)$/, '十折叠 #$1'],
    ...
    [/^Synergied \#([\d\.,]+)$/, '协同 #$1'],
    [/^Synergied (.+) \#([\d\.,]+)$/, '协同 $1 #$2'],
    [/^Greater Boost \#([\d\.,]+)$/, '更强的提升 #$1'],
    [/^Greater Boost (.+) \#([\d\.,]+)$/, '更强的提升 $1 #$2'],
    [/^Nightmare Trial ([\d\.,]+)$/, '梦魇实验 $1'],
    [/^Lvl ([\d\.,]+)$/, '等级 $1'],
    [/^Day ([\d\.,]+)$/, '天数 $1'],
    [/^level ([\d\.,]+) \/ ([\d\.,]+)$/, '等级 $1 \/ $2'],
    [/^level: ([\d\.,]+) \/ ([\d\.,]+)$/, '等级 $1 \/ $2'],
    ...
    [/^Cost: (.+) points$/, '成本：$1 点数'],
    [/^Void: (.+) \[\+(.+)\/s\] \[\/(.+) Void\]$/, '星空：$1 [+$2/秒] [/$3 虚空]'],
    [/^Super Energy: (.+) \[\+(.+)\/s\] \[\/(.+) Energy\]$/, '超级能量：$1 [+$2/秒] [/$3 能量]'],
    [/^Ascend \[Ascension Points: (.+)\]$/, '转生 [转生点数: $1]'],
    [/^Reward: (.+)x Void$/, '奖励：$1x 虚空'],
    [/^Req: (.+) elves$/, '要求：$1 精灵'],
    [/^Req: (.+) \/ (.+) elves$/, '要求：$1 \/ $2 精灵'],
    [/^Usages: (\d+)\/$/, '用途：$1\/'],
    [/^workers: (\d+)\/$/, '工人：$1\/'],
